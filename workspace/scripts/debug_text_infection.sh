...
# =============================================================================
# Script de diagnostic pour l'infection des fichiers texte
# =============================================================================
...
print_header "Diagnostic de l'infection des fichiers texte"
...
# Vérifier qu'ils ne sont pas encore infectés
...
for file in /tmp/test2/sample.*; do
    if [ -f "$file" ]; then
        ...
        if strings "$file" | grep -q "Famine"; then
            print_error "Le fichier $(basename $file) est déjà infecté !"
        else
            print_success "Fichier $(basename $file) propre"
        fi
    fi
done
